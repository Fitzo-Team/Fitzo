...
const getMonthName = (date: Date) => new Intl.DateTimeFormat('pl-PL', { month: 'long', year: 'numeric' }).format(date);
...
export default function JournalScreen() {
  ...
  const goToAdd = (mealType: MealType) => {
    router.push({ pathname: '/add', params: { initialMeal: mealType, date: currentDate.toISOString() } });
  };
        ...
        {items.length === 0 ? (
          ...
          items.map((item, index) => (
            <View key={`${item.id || index}`} className="flex-row justify-between items-center bg-brand-card p-4 rounded-2xl mb-2 border border-brand-accent shadow-sm">
              ...
              <TouchableOpacity className="p-2 bg-brand-dark/50 rounded-lg" onPress={() => item.id && removeFood(dateKey, mealType, item.id)}>
  ...
  return (
    <View className="flex-1 bg-brand-dark">
      ...
      <View className="bg-brand-card rounded-b-[35px] pt-12 pb-6 shadow-2xl shadow-brand-dark/80 border-b border-brand-accent overflow-hidden relative">
        ...
        <Text className="text-center text-brand-text font-bold text-lg mb-4 capitalize">{getMonthName(currentDate)}</Text>
        ...
        <View className="flex-row justify-around px-2 mb-4">
          {calendarDays.map((d, index) => {
             ...
             return (
                <TouchableOpacity key={index} onPress={() => setCurrentDate(d)} className={`items-center py-3 w-12 rounded-2xl border ${isSelected ? 'bg-brand-vivid border-brand-vivid shadow-lg shadow-brand-primary' : 'bg-transparent'} ${isToday && !isSelected ? 'border-brand-text/50' : 'border-transparent'}`}>
                  <Text className={`text-xs mb-1 ${isSelected ? 'text-white font-bold' : 'text-brand-muted'}`}>{getDayName(d)}</Text>
                  <Text className={`text-lg font-bold ${isSelected ? 'text-white' : 'text-brand-text'}`}>{d.getDate()}</Text>
        ...
        <View className="flex-row justify-between items-center px-6">
           <View>
             <Text className="text-brand-muted text-xs uppercase font-bold tracking-widest">{remaining < 0 ? "Przekroczono" : "Pozostało"}</Text>
        ...
        <TouchableOpacity activeOpacity={1} onPress={() => setIsOptionsOpen(false)} className="flex-1 bg-black/60 justify-end">
            <TouchableWithoutFeedback>
                <View className="bg-brand-card rounded-t-3xl p-6 border-t border-brand-accent">
                    ...
                    <TouchableOpacity className="flex-row items-center p-4 bg-brand-dark rounded-xl mb-3" onPress={() => { setIsOptionsOpen(false); setIsEditMode(true); }}>
                        <Ionicons name="list" size={24} color="#E0AAFF" />
                        <Text className="text-brand-text ml-4 font-bold">Zmień widoczne posiłki</Text>
                    </TouchableOpacity>

                    <TouchableOpacity className="flex-row items-center p-4 bg-red-500/10 rounded-xl mb-6" 
                      onPress={() => { Alert.alert("Wyczyścić?", "To usunie wszystkie posiłki z tego dnia.",
                      [{ text: "Tak", style: "destructive" }, { text: "Nie" }]); setIsOptionsOpen(false); }}>
              ...
              <View className="bg-brand-card rounded-t-3xl p-6 border-t border-brand-accent h-[70%]">
                  ...
                  <ScrollView>
                    {ALL_MEAL_SECTIONS.map(meal => {
                        ...
                        return (
                            <TouchableOpacity key={meal} className={`flex-row items-center p-4 mb-2 rounded-xl border 
                            ${isActive ? 'bg-brand-primary/20 border-brand-primary' : 'bg-brand-dark border-brand-dark'}`} 
