...
import { View, Text, ScrollView, Image, TouchableOpacity, FlatList, ActivityIndicator, Modal, Alert } from 'react-native';
...
const RecipeCard = ({ item, onPress }: { item: RecipeListDto, onPress: (id: string) => void }) => {
    ...
    return (
      ...
      >
        ...
        <View className="p-3">
          <Text className="text-brand-text font-bold text-base mb-2 h-12" numberOfLines={2}>{item.name}</Text>
          <Text className="text-brand-vivid font-bold text-sm mb-2">{item.totalCalories?.toFixed(0) || 0} kcal</Text>
          <View className="flex-row justify-between pt-2 border-t border-brand-dark/50">
              <View className="items-center"><Text className="text-brand-muted text-[10px] uppercase font-bold">B</Text>
              <Text className="text-white text-xs font-medium">{item.totalProtein?.toFixed(0) || 0}</Text></View>
              <View className="items-center"><Text className="text-brand-muted text-[10px] uppercase font-bold">T</Text>
              <Text className="text-white text-xs font-medium">{item.totalFat?.toFixed(0) || 0}</Text></View>
              <View className="items-center"><Text className="text-brand-muted text-[10px] uppercase font-bold">W</Text>
              <Text className="text-white text-xs font-medium">{item.totalCarbs?.toFixed(0) || 0}</Text></View>
          </View>
        </View>
      </TouchableOpacity>
    );
...
const RecipeDetailModal = ({ 
    visible, recipeId, onClose, onAdd 
}: { 
    visible: boolean, recipeId: string | null, onClose: () => void, 
    onAdd: (recipe: any, meal: MealType, portions: number) => Promise<void> 
}) => {
    ...
    const handleAdd = async () => {
        ...
        setAdding(true);
        try {
            await onAdd(details, selectedMeal, portions);
            Alert.alert("Sukces", "Dodano przepis do dziennika!");
            onClose();
        } catch (e) {
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 150 }}>
                        ...
                        <View className="px-5 -mt-10">
                            ...
                            <View className="flex-row bg-brand-card p-4 rounded-2xl border border-brand-accent justify-around mb-6 shadow-lg">
                                <View className="items-center"><Text className="text-brand-vivid font-bold text-xl">
                                  {details.totalCalories?.toFixed(0)}</Text><Text className="text-brand-muted text-xs uppercase">kcal</Text></View>
                                <View className="h-full w-[1px] bg-brand-dark" />
                                <View className="items-center"><Text className="text-white font-bold text-lg">
                                  {details.totalProtein?.toFixed(1)}g</Text><Text className="text-brand-muted text-xs uppercase">Białko</Text></View>
                                <View className="items-center"><Text className="text-white font-bold text-lg">
                                  {details.totalFat?.toFixed(1)}g</Text><Text className="text-brand-muted text-xs uppercase">Tłuszcz</Text></View>
                                <View className="items-center"><Text className="text-white font-bold text-lg">
                                  {details.totalCarbs?.toFixed(1)}g</Text><Text className="text-brand-muted text-xs uppercase">Węgle</Text></View>
                            </View>

                            <Text className="text-white text-xl font-bold mb-3">Dodaj do dziennika</Text>
                            <View className="bg-brand-card rounded-2xl border border-brand-accent p-4 mb-6">
                                <Text className="text-brand-muted text-xs mb-2">Wybierz posiłek:</Text>
                                    ...
                                    {Object.values(MealType).map((meal) => (
                                        <TouchableOpacity 
                                            ...
                                            className={`px-3 py-2 rounded-full border mr-2 
                                              ${selectedMeal === meal ? 'bg-brand-primary border-brand-primary' : 'bg-brand-dark border-brand-muted/30'}`}
                                        >
                                            <Text className={`text-xs ${selectedMeal === meal ? 'text-white font-bold' : 'text-brand-muted'}`}>
                                                {MEAL_LABELS[meal as MealType]}
                                            </Text>
                                        </TouchableOpacity>
                                    ))}
                                </ScrollView>
                                ...
                                <View className="flex-row items-center justify-between mb-4">
                                    <Text className="text-brand-text font-bold">Liczba porcji:</Text>
                                    <View className="flex-row items-center gap-4">
                                        <TouchableOpacity onPress={() => setPortions(p => Math.max(0.5, p - 0.5))} 
                                        className="bg-brand-dark p-2 rounded-lg border border-brand-muted/30">
                                          <Ionicons name="remove" size={20} color="white" /></TouchableOpacity>
                                        <Text className="text-white text-xl font-bold w-8 text-center">{portions}</Text>
                                        <TouchableOpacity onPress={() => setPortions(p => p + 0.5)} 
                                        className="bg-brand-dark p-2 rounded-lg border border-brand-muted/30">
                                          <Ionicons name="add" size={20} color="white" /></TouchableOpacity>
                                    </View>
                                </View>

                                <TouchableOpacity 
                                    className="bg-brand-vivid py-4 rounded-xl items-center shadow-lg"
                                    onPress={handleAdd}
                                    disabled={adding}
                                >
                                    {adding ? <ActivityIndicator color="white" /> : <Text className="text-white font-bold text-lg">Dodaj do dziennika</Text>}
                                </TouchableOpacity>
                            </View>
                                    ...
                                    details.ingredients.map((ing, index) => (
                                        ...
                                        ${index < (details.ingredients?.length || 0) - 1 ? 'border-b border-brand-dark' : ''}`}>
                                            <View className="flex-1">
                                                ...
                                                <Text className="text-brand-muted text-xs">{ing.product?.calories} kcal</Text>
                                            </View>
...
export default function RecipesScreen() {
  ...
  const { addRecipeToDiary } = useFood();
  ...
  const handleAddRecipe = async (recipe: any, meal: MealType, portions: number) => {
      const today = new Date().toISOString();
      await addRecipeToDiary(today, meal, recipe, portions);
  };
    ...
    return (
        <View className="mb-8">
          <View className="flex-row justify-between items-center px-5 mb-4">
            ...
            <TouchableOpacity><Text className="text-brand-primary text-sm font-medium">Zobacz wszystkie</Text></TouchableOpacity>
          </View>
          <FlatList
            data={data}
  ...
  if (loading && recipes.length === 0) {
      return (<View className="flex-1 bg-brand-dark justify-center items-center"><ActivityIndicator size="large" color="#E0AAFF" /></View>);
  }
  ...
  return (
    <View className="flex-1 bg-brand-dark pt-12">
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        {myRecipes.length > 0 ? renderSection('Twoje przepisy', myRecipes) : (
